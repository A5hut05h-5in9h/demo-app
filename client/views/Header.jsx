...
export default class Header extends React.Component {
	...
	getProfilePic(username) {
		let th = this;
		Request.get(`/dashboard/getimage`)
	...
	getNotifications(username) {
		let th = this;
		Request.get(`/dashboard/notifications?username=${username}`)
			.set({ Authorization: localStorage.getItem('token') })
			.end(function(err, res) {
				th.setState({
	...
	dropNotification(index) {
		...
		let notifications = th.state.notifications;
		Request.post('/dashboard/deletenotification')
			.set({ Authorization: localStorage.getItem('token') })
	...
	logout() {
		let th = this;
		Request.post('/dashboard/lastlogin')
			.set({ Authorization: localStorage.getItem('token') })
	...
	render() {
		...
		return (
			<div style={styles.header}>
				...
				<AppBar
					title={<span style={styles.title}>demo-app</span>}
					...
					iconElementRight={
						<div>
							...
							>
								<List>
									<ListItem
										primaryText="Update Profile Pic"
										onClick={() =>
											th.toggleDialog('ProfilePic')
										}
										leftIcon={<ProfilePicIcon />}
										style={{ color: '#757575' }}
									/>
									<ListItem
										primaryText="Change Password"
										onClick={() =>
											th.toggleDialog('ChangePassword')
										}
				...
				>
					<ChangePassword
						...
						handleClose={() => th.toggleDialog('ChangePassword')}
					/>
				</Dialog>
